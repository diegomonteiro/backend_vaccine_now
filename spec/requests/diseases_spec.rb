 require 'rails_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to test the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator. If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails. There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.

RSpec.describe "/diseases", type: :request do
  
  # Disease. As you add validations to Disease, be sure to
  # adjust the attributes here as well.
  let(:valid_attributes) {
    skip("Add a hash of attributes valid for your model")
  }

  let(:invalid_attributes) {
    skip("Add a hash of attributes invalid for your model")
  }

  describe "GET /index" do
    it "renders a successful response" do
      Disease.create! valid_attributes
      get diseases_url
      expect(response).to be_successful
    end
  end

  describe "GET /show" do
    it "renders a successful response" do
      disease = Disease.create! valid_attributes
      get disease_url(disease)
      expect(response).to be_successful
    end
  end

  describe "GET /new" do
    it "renders a successful response" do
      get new_disease_url
      expect(response).to be_successful
    end
  end

  describe "GET /edit" do
    it "render a successful response" do
      disease = Disease.create! valid_attributes
      get edit_disease_url(disease)
      expect(response).to be_successful
    end
  end

  describe "POST /create" do
    context "with valid parameters" do
      it "creates a new Disease" do
        expect {
          post diseases_url, params: { disease: valid_attributes }
        }.to change(Disease, :count).by(1)
      end

      it "redirects to the created disease" do
        post diseases_url, params: { disease: valid_attributes }
        expect(response).to redirect_to(disease_url(Disease.last))
      end
    end

    context "with invalid parameters" do
      it "does not create a new Disease" do
        expect {
          post diseases_url, params: { disease: invalid_attributes }
        }.to change(Disease, :count).by(0)
      end

      it "renders a successful response (i.e. to display the 'new' template)" do
        post diseases_url, params: { disease: invalid_attributes }
        expect(response).to be_successful
      end
    end
  end

  describe "PATCH /update" do
    context "with valid parameters" do
      let(:new_attributes) {
        skip("Add a hash of attributes valid for your model")
      }

      it "updates the requested disease" do
        disease = Disease.create! valid_attributes
        patch disease_url(disease), params: { disease: new_attributes }
        disease.reload
        skip("Add assertions for updated state")
      end

      it "redirects to the disease" do
        disease = Disease.create! valid_attributes
        patch disease_url(disease), params: { disease: new_attributes }
        disease.reload
        expect(response).to redirect_to(disease_url(disease))
      end
    end

    context "with invalid parameters" do
      it "renders a successful response (i.e. to display the 'edit' template)" do
        disease = Disease.create! valid_attributes
        patch disease_url(disease), params: { disease: invalid_attributes }
        expect(response).to be_successful
      end
    end
  end

  describe "DELETE /destroy" do
    it "destroys the requested disease" do
      disease = Disease.create! valid_attributes
      expect {
        delete disease_url(disease)
      }.to change(Disease, :count).by(-1)
    end

    it "redirects to the diseases list" do
      disease = Disease.create! valid_attributes
      delete disease_url(disease)
      expect(response).to redirect_to(diseases_url)
    end
  end
end
